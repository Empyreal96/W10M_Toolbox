...
namespace W10M_Toolbox
{
    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        // processes for dd for windows commands
        Process dmainos = new Process();
        Process ddata = new Process();
        Process defi = new Process();

        //values for storing fetched disk info for wp
        int MAINOS_INDEX;
        int DATA_INDEX;
        int EFIESP_INDEX;
        string EFIESP_DRIVE;
        ...
        public MainWindow()
        {
            ...
            InitializeComponent();
            // worker = partition dump BackgroundWorker
            ...
            // worker 2 = emmc dump BackgroundWorker
            ...
            //Hooks onto process window handle for notifying the app when a usb device connected
        ...
        public void GetMassStorageDrive()
        {
            ...
            foreach (ManagementObject logical in coll)
            {
                ...
                foreach (ManagementObject partition in logical.GetRelated("Win32_DiskPartition"))
                {
                    ...
                    {
                        // "PROD_VIRTUAL_DISK" is added for debugging purposes (test without phone connected using vhd)
                            ...
                            drive["PNPDeviceID"].ToString().Contains("VEN_MSFT&PROD_PHONE_MMC_STOR", StringComparison.CurrentCulture) || drive["PNPDeviceID"].ToString().Contains("VEN_MSFT&PROD_VIRTUAL_DISK", StringComparison.CurrentCulture))
                        {
                            ...
                            // added finding EFIESP and Data drive info
        ...
        public void MassStorage()
        {
            try
            {
                ...
                else
                {
                    ...
                    //load hive and open the mounted subkey
                    ...
                    //valus for the setting registry keys
        ...
        /* 
         * 
         * Below was an old test at retrieving/checking WP disk info
         * 
         * 
         * 
         * // EFIESP_PHYSICAL, 0
        ...
        // Run check on a few things before closing, if the window is closed by user
        ...
        #region USB Event Notification

        /// <summary>
        /// Runs when notification to the application window when USB event detected
        /// </summary>
        /// <param name="e"></param>
        ...
        #region Check for connected device 

        private void RefreshDevice_Click(object sender, RoutedEventArgs e)
        {
            CheckForDevice();
        }
        // Check if device is connected, calling iutool and WPInternals Mass Storage checks
        ...
        #region 1st tab button click events (Home tab)
        ...
        /// <summary>
        /// Read device logs with getdulogs, and fetch the update list the user selected
        /// </summary>
        ...
        /// <summary>
        /// Push downloaded update cabs with iutool
        /// </summary>
        ...
        #region 3rd tab events (Registry, bcd, click events)
        ...
        /// <summary>
        /// Make sure phone registry isn't mounted
        /// </summary>
        ...
        // Check BCD values for the device config (flight signing)
        ...
        // Modify flight signing optinon (on/off)
        ...
        // Click event for saving modified changes for device config
        ...
        #region 4th tab events (Partition dumps)

        // Start fulll emmc dump 
        ...
        // https://github.com/simonkaps/ddresize/blob/master/Program.cs    < Can't remember if I used this or not

        /// <summary>
        /// Ask for output folder and start background worker for partition dumps
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        // below just notify that partition box checked
        ...
        #region Background worker (partition dump)
        ...
        // Background Worker task for Partition dumps, MainOS, Data and EFIESP currently
        ...
        #region Background Worker (emmc dump)
        ...
        #region FFU Conversion events

        // Load FFU click event
        ...
        // load ffu into a readable stream (thanks to ffuconvert on github)
        ...
        /// <summary>
        /// Read ffu stream and retrieve details
        /// </summary>
        ...
        /// <summary>
        /// Read and convert the ffu stream stored globally to .vhdx
        /// 
        /// Issue: only wp8 ffu are supported, Mcrosoft changed something in the ffu formatting preventing conversion
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// On Click, Ask save location and call process to start
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Enable/Disable UI Elements for Mass Storage Mode in backup tab
        /// 
        /// TODO: Extend for each page with if conditions?
        /// </summary>
